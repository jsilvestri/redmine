...
class StuffToDo < ActiveRecord::Base
  ...
  def self.remove_associations_to(associated_object)
    ...
    user_ids.uniq.each do |user_id|
      ...
      threshold = 10 # Setting.plugin_stuff_to_do_plugin['threshold']
  ...
  def self.reorder_list(user, ids)
    ids ||= []
    id_position_mapping = arr_to_hash(ids)
  ...
  def self.use_setting
    USE.index(true) #Setting.plugin_stuff_to_do_plugin['use_as_stuff_to_do'])
  end

  def self.conditions_for_available(filter_by)
    conditions_str = ''
    conditions_vals = Array.new()

    conditions_str = "#{IssueStatus.table_name}.is_closed = ?"
    conditions_vals << false

    conditions_str << ' AND '
    conditions_str << "#{Project.table_name}.status = ?"
    conditions_vals << Project::STATUS_ACTIVE

    case 
    when filter_by.is_a?(User)
      conditions_str << ' AND '
      conditions_str << "assigned_to_id = ?"
      conditions_vals << filter_by.id   
    when filter_by.is_a?(IssueStatus), filter_by.is_a?(Enumeration)
      table_name = filter_by.class.table_name
      conditions_str << ' AND '
      conditions_str << "#{table_name}.id = (?)"
      conditions_vals << filter_by.id
    end

    # prepend conditions string to get proper rails conditions format
    conditions_vals.unshift(conditions_str)
  end
end
